...
def evaluate(model: ContinualModel, dataset: ContinualDataset, last=False, eval_ema=False, ema_model=None) -> Tuple[list, list]:
    ...
    if eval_ema:
        curr_model = getattr(model, ema_model)
...
def train(model: ContinualModel, dataset: ContinualDataset,
    ...
    for t in range(dataset.N_TASKS):
        ...
        for epoch in range(args.n_epochs):
            ...
            for i, data in enumerate(train_loader):
                ...
                if len(data) == 4:
                    inputs, labels, not_aug_inputs, super_class = data
                    inputs, labels, not_aug_inputs, super_class = inputs.to(model.device), labels.to(model.device), not_aug_inputs.to(model.device), super_class.to(model.device)
                    loss = model.observe(inputs, labels, not_aug_inputs, super_class)
                elif hasattr(dataset.train_loader.dataset, 'logits'):
    ...
    if args.save_model:
        # Save the model
        model_dir = os.path.join(args.output_dir, "saved_models", dataset.NAME, args.experiment_id)
        os.makedirs(model_dir, exist_ok=True)
        torch.save(model.net, os.path.join(model_dir, f'model.ph'))
