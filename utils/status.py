...
def progress_bar_context(i: int, max_iter: int, loss: float) -> None:
    """
    Prints out the progress bar on the stderr file.
    :param i: the current iteration
    :param max_iter: the maximum number of iteration
    ...
    if not (i + 1) % 10 or (i + 1) == max_iter:
        ...
        print('\r[ {} ] Learning Context|{}| loss: {}'.format(
            datetime.now().strftime("%m-%d | %H:%M"),
            progress_bar,
            round(loss, 8)
        ), file=sys.stderr, end='', flush=True)
...
def progress_bar_context_v1(i: int, max_iter: int, loss: float, sup_loss: float, cont_loss:float, cons_loss:float,) -> None:
    """
    Prints out the progress bar on the stderr file.
    :param i: the current iteration
    :param max_iter: the maximum number of iteration
    ...
    if not (i + 1) % 10 or (i + 1) == max_iter:
        ...
        print('\r[ {} ] Learning Context|{}| loss: {}, l_s: {}, l_cont:{} l_cons:{}, '.format(
            datetime.now().strftime("%m-%d | %H:%M"),
            progress_bar,
            round(loss, 8),
            round(sup_loss, 8),
